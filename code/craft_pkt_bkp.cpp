...
void craft_udp_packet(char* s_addr, char *d_addr, int flow_id, int seq) {
    ...
    /* Ethernet header */
    pkt->eth_hdr.ether_shost[0] = 0x86;
    pkt->eth_hdr.ether_shost[1] = 0x47;
    pkt->eth_hdr.ether_shost[2] = 0xf9;
    pkt->eth_hdr.ether_shost[3] = 0xda;
    pkt->eth_hdr.ether_shost[4] = 0xcb;
    pkt->eth_hdr.ether_shost[5] = 0x4f;

    pkt->eth_hdr.ether_dhost[0] = 0x8e;
    pkt->eth_hdr.ether_dhost[1] = 0x93;
    pkt->eth_hdr.ether_dhost[2] = 0x9b;
    pkt->eth_hdr.ether_dhost[3] = 0xbb;
    pkt->eth_hdr.ether_dhost[4] = 0x2c;
    pkt->eth_hdr.ether_dhost[5] = 0x69;
...
void send_packets(int max_flows, int max_seq) {
    ...
    for(int cycle = 0; cycle < MAX_CYCLES; cycle++) {
        double curr_ts = 0;
        for (int iter = 0; iter < packet_send_time[cycle].size(); iter++) {
            // Sleep (usec) until time to send next pkt
            double x = (packet_send_time[cycle][iter].second - curr_ts); /*/ 1000.0; */
            usleep(x);
